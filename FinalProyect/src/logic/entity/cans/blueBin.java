...
public class blueBin extends StaticEntity{
	
	
	public blueBin(Handler handler, float x, float y, int width, int height) {
	...
	public void tick() {
		...
		if (handler.getWorld().getPlayer().getResidue() != null) {
			...
			if (handler.getWorld().getPlayer().getCollisionsBounds(handler.getWorld().getPlayer().getxMove(),
				...
				if (handler.getWorld().getPlayer().getResidue().getTypeResidue().equals("papel") || handler.getWorld().getPlayer().getResidue().getTypeResidue().equals("carton") ) {
	...
	public void render(Graphics g) {
		g.drawImage(Assets.blueBin, (int)x, (int)y, width, height, null);
