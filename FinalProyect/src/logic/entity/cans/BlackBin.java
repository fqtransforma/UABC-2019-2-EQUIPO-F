...
public class BlackBin extends StaticEntity {

	public BlackBin(Handler handler, float x, float y, int width, int height) {
	...
	public void tick() {
		...
		if (handler.getWorld().getPlayer().getResidue() != null) {
			...
			if (handler.getWorld().getPlayer().getCollisionsBounds(handler.getWorld().getPlayer().getxMove(),
				...
				if (handler.getWorld().getPlayer().getResidue().getTypeResidue().equals("aluminio")) {
	...
	public void render(Graphics g) {
		g.drawImage(Assets.blackBin, (int)x, (int)y, width, height, null);
